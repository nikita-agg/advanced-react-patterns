...
const Usage = () => {
  const [state, setState] = useState(INITILA_STATE)
  const handleClapClick = (clapState) => {
    setState(() => ({
      isClicked: true,
      count: Math.min(state.count + 1, MAXIMUM_USER_CLAP_VAL),
      countTotal:
      state.count < MAXIMUM_USER_CLAP_VAL
      ? state.countTotal + 1
      : state.countTotal
    }))
  }
  ...
  return (
    <div style={{ width: '100%' }}>
      <MediumClap onClap={handleClapClick} className={userCustomStyles.clap} values={state}>
